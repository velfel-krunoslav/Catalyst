...
contract Products{
    ...
    function createProduct(string memory _productName, uint _price_nominator, uint _price_denominator, uint _discount_percentage, string memory _assetUrls, uint _classif, uint _quantifier, string memory _desc, uint _sellerId, uint _categoryId, uint _inStock) public{

        if (_classif > 2)
            _classif = 0;
        products[productsCount++] = Product(idCount++, _productName, _price_nominator, _price_denominator, _discount_percentage, _assetUrls, _classif, _quantifier, _desc, _sellerId, _categoryId, _inStock);
    ...
    function editProduct(uint _id, string memory _productName, uint _price_nominator, uint _price_denominator, string memory _assetUrls, uint _classif, uint _quantifier, string memory _desc, uint _categoryId, uint _inStock) public {
        for (uint i= 0; i < productsCount; i++) {
            if (products[i].id == _id){
                products[i].name = _productName;
                products[i].price_numerator = _price_nominator;
                products[i].price_denominator = _price_denominator;
                products[i].assetUrls = _assetUrls;
                products[i].classification = _classif;
                products[i].quantifier = _quantifier;
                products[i].decs = _desc;
                products[i].categoryId = _categoryId;
                products[i].inStock = _inStock;
                break;
            }
        }
    }
